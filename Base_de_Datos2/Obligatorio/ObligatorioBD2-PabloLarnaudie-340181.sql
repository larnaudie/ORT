...
--	Parámetros de entrada:
    ...
    @IdProd INT,
    @IdEmp INT,
    @CostoRepara MONEY -- Estuve investigando y averigue que puedo colocarle money como tipo de dato.
AS
BEGIN
--	La unidad (@NumSerie, @IdProd) debe existir en la tabla Unidad.
    IF NOT EXISTS (
    ...
    BEGIN
        PRINT 'La unidad no existe, error.'
        RETURN
    END
	--	Un empleado no puede registrar más de una reparación para la misma unidad
		--	en el mismo día.
    ...
    BEGIN
        PRINT 'El empleado no existe, error.'
        RETURN
    END
	--	El costo de reparación no puede ser negativo.
    IF @CostoRepara < 0
    BEGIN
        PRINT 'El costo de reparación no puede ser negativo, error.'
        RETURN
    END
    IF EXISTS (
        SELECT 1
        FROM Repara
        WHERE NumSerie = @NumSerie AND IdProd = @IdProd AND IdEmp = @IdEmp
        AND CONVERT(DATE, FchRepara) = CONVERT(DATE, GETDATE())
    )
    BEGIN
        PRINT 'El empleado ya registró una reparación para esta unidad hoy, error'
        RETURN
    END
		--	Insertar la nueva reparación en la tabla Repara, con el estado 'Iniciado' y la
		--	fecha/hora actual.
    INSERT INTO Repara (
        NumSerie, IdProd, IdEmp, FchRepara, CostoRepara, StsRepara
    )
    VALUES (
        @NumSerie, @IdProd, @IdEmp, GETDATE(), @CostoRepara, 'Iniciado'
    )

	--	Retornar un mensaje indicando el éxito.
    PRINT 'Reparación registrada correctamente.'
END

--Voy a probar un caso de exito y de error con el primer numero de serie:
...
-- Crea una función escalar llamada fn_CalcularTiempoReparacion, que reciba el
--	número de serie y el ID del producto y devuelva el tiempo total (en días) que ha	
--	estado en reparación.
...
--Voy a probarlo, no se por que se subraya de rojo pero funciona.
			--	número de serie y el ID del producto y devuelva el tiempo total (en días) que ha	
			--	estado en reparación.
SELECT dbo.fn_CalcularTiempoReparacion('SN010AUR02', 5) AS DiasDeReparacion
...
AS
BEGIN
	--	Registrar el cambio en una tabla llamada HistoricoReparacion
	--	Insertar en esta tabla el IdRepara, NumSerie, IdProd, el estado anterior, el
	--	estado nuevo y la fecha del cambio.
    INSERT INTO HistoricoReparacion (
        IdRepara, NumSerie, IdProd, EstadoAnterior, EstadoNuevo
    )
    SELECT
        i.IdRepara,
        i.NumSerie,
    ...
    AND i.StsRepara <> d.StsRepara
END

--Lo pruebo
SELECT * FROM Repara WHERE IdRepara = 1
SELECT * FROM HistoricoReparacion WHERE IdRepara = 1

--le cambié el estado de Terminado a Cancelado y de Cancelado a Terminado
UPDATE Repara
...
--Intento ver si la tabla se actualiza
SELECT * FROM HistoricoReparacion WHERE IdRepara = 1;
...
BEGIN
    ...
    BEGIN
			--	Si el estado es "En testing" o "Terminado", debe bloquear la eliminación y
			--	mostrar un mensaje de error.
        PRINT('No se pueden eliminar reparaciones con estado "Terminado" o "En testing".')
        RETURN;
    END
	--	Si la eliminación es permitida, debe registrarse en una tabla de auditoría
	--	HistoricoEliminacionReparaciones, con los siguientes datos:
...
--  Pruebo si permite eliminar sin bloquear.
select * from Repara --> Miro la tabla y uso uno que tenga estado Iniciado, no terminado, sino lo bloquea
DELETE FROM Repara WHERE IdRepara = 50

-- Verifico y chequeo el historial a ver si esta registrado
SELECT * FROM HistoricoEliminacionReparaciones WHERE IdRepara = 47--> Actualizo el IdRepara con el IdRepara que use arriba

-- Voy a probar si bloquea correctamente
select * from Repara --> Miro la tabla y uso uno que tenga estado Terminado para que lo bloquee
DELETE FROM Repara WHERE IdRepara = 2
select * from Repara
...
CREATE VIEW ReparacionesActivas_vista AS
SELECT
    r.IdRepara,
    r.NumSerie,
    r.IdProd,
...
SELECT * FROM ReparacionesActivas_vista