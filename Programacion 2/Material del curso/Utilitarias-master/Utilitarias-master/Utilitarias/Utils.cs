...
namespace Utilitarias
{
    ...
    {
        static int LeerNumeroEnRango(string prompt,int min, int max)
        {
            int opcion;
            while (true)
            {
        ...
        public static int LeerEnum(string prompt, Type enumType) 
        {
            ...
            {
                Console.WriteLine($"({i++}) {itemEnum}");   
            }
            int seleccion =LeerNumeroEnRango(prompt,1,Enum.GetNames(enumType).Length);
            return seleccion;
        }
