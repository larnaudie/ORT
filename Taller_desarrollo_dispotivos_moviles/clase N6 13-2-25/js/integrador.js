...
function previaListado(){
    fetch (`https://ort-tallermoviles.herokuapp.com/api/pedidos`,{
        ...
        .then(function(informacion){
          ...
          mostrarListado(informacion.data)
        })
        .catch(function(error){
        console.log(error)
        })
...
function mostrarListado(listaPedidos){

    let misPedidos=""
    for (let unPedido of listaPedidos){
        misPedidos+=`
        <ion-item>
                <ion-label>
                    <h3>Id: ${unPedido._id}</h2>
                    <h3>Producto: ${unPedido.producto.nombre}</h3>
                    <p>Precio: ${unPedido.producto.precio}</p>
                </ion-label>
                <ion-button onclick="eliminarPedido()">Eliminar</ion-button>
        </ion-item>
       `
    }
    document.querySelector("#contenedorListado").innerHTML=misPedidos
}

function eliminarPedido(){
    alert ("aca eliminamos")
}